...
class CDPTokenAuthResponse(TokenResponse):
    ...
    def __repr__(self) -> str:
        return (
            f"{CDPTokenAuthResponse.__name__}"
            f"{{Token: {self.token}, Expires At: {self.expires_at}}}"
        )
...
class CDPAccessKeyV2TokenAuth(CDPAuth):
    ...
    def __init__(self,
                ...
                altus_iam_endpoint: Optional[str] = None) -> None:
        self.service_id = service_id
        self.cdp_cred = cdp_cred
        self.cdp_describe_service_endpoint = cdp_endpoint if cdp_endpoint \
                                        else self.CDP_ENDPOINT_DEFAULT
        ...
        self.altus_iam_gen_workload_auth_endpoint = altus_iam_endpoint \
                                        if altus_iam_endpoint \
                                        else self.ALTUS_IAM_ENDPOINT_DEFAULT
    ...
    def _generate_workload_auth_token(self, env_crn: str,
                                      workload_name: str) -> CDPTokenAuthResponse:
        headers = {'Content-Type': 'application/json'}
        # make_request only accepts a string for the request body
