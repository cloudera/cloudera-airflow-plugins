...
from airflow.hooks.http_hook import HttpHook  # type: ignore
...
from cloudera.cdp.security.cde_security import (BearerAuth, CDEAPITokenAuth,
...
class CDEHook(BaseHook): # type: ignore
    ...
    def __init__(
            ...
            api_timeout: int = DEFAULT_API_TIMEOUT) -> None:
        """
        ...
        self.connection = CDEConnection.from_airflow_connection(
            airflow_connection)
    ...
    def _do_api_call(self,
        ...
        self.log.debug('Executing API call: (Method: %s, Endpoint: %s, Parameters: %s)',
        ...
        try:
            ...
            if method.upper() == 'GET':
                response = http.run_with_advanced_retry(
                    data=params, **common_kwargs)
            else:
                response = http.run_with_advanced_retry(
                    json=params, **common_kwargs)
    ...
    def get_cde_token(self) -> str:
        ...
        access_key, private_key = self.connection.access_key, self.connection.private_key
        ...
        try:
            ...
            cdp_auth = CDPAccessKeyV2TokenAuth(cde_vcluster.get_service_id(), cdp_cred,
            ...
            cache_mech = EncryptedFileTokenCacheStrategy(CDETokenAuthResponse,
                                        encryption_key=cdp_auth.get_auth_secret(),
                                        **cache_mech_extra_kw)

            cde_auth = CDEAPITokenAuth(cde_vcluster, cdp_auth, cache_mech,
            ...
            self.log.error("Failed to get the cde auth token for the connection %s, error: %s",
                self.cde_conn_id, err)
            raise CDEHookException(err) from err
    ...
    def submit_job(
            ...
            proxy_user: Optional[str] = None) -> int:
        """
