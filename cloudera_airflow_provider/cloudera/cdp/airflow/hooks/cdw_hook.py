...
class CDWHook(HiveCliHook):
    ...
    def _prepare_cli_cmd(self, hide_secrets=False):
        ...
        jdbc_url = "jdbc:{jdbc_backend}://{host}{port_string}/{schema}".format(
            jdbc_backend=jdbc_backend, host=conn.host, schema=conn.schema,
            port_string=CDWHook.get_port_string(conn))
        ...
        jdbc_url = self.add_parameter_to_jdbc_url(conn.extra_dejson, jdbc_url, 'ssl',
            CDWHook.get_ssl_parameter(conn))
    ...
    def add_extra_parameters(self, jdbc_backend, cmd_extra):
        ...
        if self.query_isolation and jdbc_backend == JDBC_BACKEND_HIVE:
            ...
            cmd_extra += ['--hiveconf', 'hive.auto.convert.join.noconditionaltask.size=2505397589']
    ...
    def get_jdbc_backend(conn):
        ...
        impala: 'impala'
        """
        return JDBC_BACKEND_IMPALA if (conn.host.find('coordinator-') == 0
                                    or conn.host.find('impala-proxy') == 0) else JDBC_BACKEND_HIVE
    ...
    def add_parameter_to_jdbc_url(extra_dejson, jdbc_url, parameter_name, default_value=None):
        ...
        return jdbc_url + ";{parameter_name}={parameter_value}".format(
            parameter_name=parameter_name, parameter_value=extra_dejson.get(parameter_name, default_value))
...
class CDWHiveMetastoreHook(CDWHook):
    ...
    def check_for_partition(self, schema, table, partition):
        ...
        :rtype: bool
        """
        hql = ("select dbs.name as db_name, tbls.tbl_name as tbl_name, partitions.part_name as part_name "
               "from partitions "
               "left outer join tbls on tbls.tbl_id = partitions.tbl_id "
               "left outer join dbs on dbs.db_id = tbls.db_id "
               "where dbs.name = '{schema}' "
               "and tbls.tbl_name = '{table}' "
               "and partitions.part_name = '{partition}';".format(schema=schema,
                                                                  table=table,
                                                                  partition=partition))
        ...
        results_without_header = CDWHiveMetastoreHook.get_results_without_header(result_lines,
    ...
    def get_table(self, table_name, db='default'):
        ...
        hql = ("select dbs.name as db_name, tbls.tbl_name as tbl_name"
        "from tbls "
        "left outer join dbs on dbs.db_id = tbls.db_id "
        "where dbs.name = '{db}' "
        "and tbls.tbl_name = '{table_name}' ".format(db=db, table_name=table_name))
    ...
    def get_tables(self, db, pattern='*'):
        ...
        "left outer join dbs on dbs.db_id = tbls.db_id "
        "where dbs.name = '{db}' "
        "and tbls.tbl_name like '{pattern}' ".format(db=db, pattern=pattern.replace("*", "%")))
    ...
    def get_databases(self, pattern='*'):
        ...
        hql = ("select dbs.name "
               "from dbs "
               "where dbs.name LIKE '{pattern}' ".format(pattern=pattern.replace("*", "%")))
    ...
    def get_results_without_header(result_lines, header):
        """
        Parses beeline output and removes noise before the given eader (e.g. SLF4J warnings).
        """
        final_list = []
        add_line = False
