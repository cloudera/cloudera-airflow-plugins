...
class CdeHookTest(unittest.TestCase):
    ...
    @mock.patch(
        'cloudera.cdp.security.cde_security.CdeApiTokenAuth.get_cde_authentication_token',
    ...
    def test_submit_job_custom_timeout_success(self, connection_mock, session_send_mock, cde_mock):
        """Ensure custom timeout is taken into account and succeeds as expected if the request does not timeout."""
    ...
    def test_submit_job_non_always_retry_exception(self, connection_mock, session_send_mock, cde_mock):
        """Ensure no retry is attempted if the request exception is not part of the ALWAYS_RETRY_EXCEPTIONS list"""
